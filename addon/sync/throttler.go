...
func (t *throttler) Init(addons []addonv1.Addon) error {
	t.lock.Lock()
	defer t.lock.Unlock()
	if t.parallelism == 0 {
		return nil
	...
	for _, addon := range addons {
		key, err := cache.MetaNamespaceKeyFunc(&addon)
		if err != nil {
			return err
		}
		if addon.Status.Lifecycle.Installed == "Running" {
