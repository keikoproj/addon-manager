...
func (r *resourceReconcile) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err == nil {
		r.log.Info(fmt.Sprintf("processing Deployment %s", namespace.Name))
	...
	if err == nil {
		r.log.Info(fmt.Sprintf("processing ServiceAccount %s", namespace.Name))
	...
	if err == nil {
		r.log.Info(fmt.Sprintf("processing ClusterRole %s", namespace.Name))
